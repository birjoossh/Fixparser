...
public class PerformanceBenchMarking {
    public static void main(String[] args) throws Exception {
        if (args.length < 3) {
            System.out.println("Usage: java ArgumentParser <maxloadFactor> <fixMessageString> <runbenchmarkParsingInParallel> <benchmarkParsingInSequence>");
            return;
        }
        try {
            int maxloadFactor = Integer.parseInt(args[0]);
            boolean runbenchmarkParsingInParallel = Boolean.parseBoolean(args[1]);
            boolean benchmarkParsingInSequence = Boolean.parseBoolean(args[2]);

            System.out.println("maxloadFactor: " + maxloadFactor);
            System.out.println("runbenchmarkParsingInParallel: " + runbenchmarkParsingInParallel);
            ...
            if (runbenchmarkParsingInParallel) {
